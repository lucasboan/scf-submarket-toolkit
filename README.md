...
- scf_toolkit.py: The core engine, containing the sequential data-processing pipeline and main(). Key functions include:
    ...
    - sjoin_gdfs_in_3081: Forces a standardized reprojection to EPSG:3081 (Texas State Mapping System, Lambert Conformal Conic)" on both ZCTA and counties GeoDataFrames (GDFs). This ensures both GDFs are in the same Coordinate Reference System (CRS) to be spatially joined, keeping only the ZIP codes intersecting the target counties.
